...
export const promiseMiddleware = (optionAction: () => PromiseMiddleWareOptions) => {
    const { before, success, failed } = optionAction();
    return <T>(promiseIn: Promise<T>) => {
        before();
        return promiseIn.then((res) => {
            success(res);
...
export const loading = (options?: PromiseLoadingOptions) => promiseMiddleware(() => {
    let spinner: any;
